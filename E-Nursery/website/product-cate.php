...
<head>
	...
	<title>E nursery 🌱  | Product Category</title>
    <style type="text/css">
  .wish{
...
if(isset($_POST['submit'])){
  $search = $_POST['search'];
    $p_selectquery = "select * from product where name like '$search%' and category = '$category' ";
}else{
    if(isset($_POST['sort'])){
       if($_POST['sort']==="highest"){
            $p_selectquery = "select * from product where category = '$category'  order by price desc ";
       }else{
           $p_selectquery = "select * from product where category = '$category'  order by price asc ";
    ...
    $p_selectquery = "select * from product where category = '$category'";
}
}
?>
...
?>
  ...
  <div class="card-body">
         <h6 class="card-title"><?php echo $p_res['name']; ?></h6>
